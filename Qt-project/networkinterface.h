...
typedef struct __FC_PARAM
{
#define FC_PARAM_REALTIME_MODE	0
#define FC_PARAM_BUFFER_MODE	1
    int nMode; /*0 Real Time Mode, Media Buffer Mode*/
    int nBufTime; /*In seconds*/
    int nContinueTime; /*In Seconds*/
    /*
     *	If report bps, the sender will adjust send bps to report bps
     */
    int bReportBps; /*0 don't report receive bps to sender, 1 do report*/
}FCParam;
//
// Parameter is a pointer to FCParam
#define NETWORK_MEDIA_OPTION_SET_FC_PARM			(NETWORK_MEDIA_OPTION_BASE+3)
#define NETWORK_MEDIA_OPTION_CLEAR_LOCAL_BUFFER		(NETWORK_MEDIA_OPTION_BASE+4)
#define NETWORK_MEDIA_OPTION_FREE_SEND_BUFFER       (NETWORK_MEDIA_OPTION_BASE+5)

typedef struct __BUFFER_PARAM
{
    int nBps;		/*Bytes Per second*/
    int nBufPackets; /*Packet number in buffer*/
    int bBuffering; /*0 playing 1 buffering*/
}BufferParam;
#define NETWORK_MEDIA_OPTION_GET_BUFFER_PARM		(NETWORK_MEDIA_OPTION_BASE+6)
/*
 *	Network
 */
...
extern "C"
{
    ...
    LIBNETWORK_EXPORT void NetworkStop();
    /*
     *	Application on WIN doesn't need call NetworkRunEventLoop
     */
    LIBNETWORK_EXPORT void NetworkRunEventLoop();
    LIBNETWORK_EXPORT void NetworkDestroyConnection(INetConnection *pCon, int nReason = 0);
    ...
    LIBNETWORK_EXPORT INetAcceptor *CreateTcpMultiMediaAcceptor(INetAcceptorSink *pSink);
    ...
    LIBNETWORK_EXPORT INetConnection *CreateUdpMediaCon(INetConnectionSink *pSink, INetAcceptor *pApt);
    ...
    LIBNETWORK_EXPORT INetConnection *CreateTcpMultiMediaCon(INetConnectionSink *pSink, int nConNum);
